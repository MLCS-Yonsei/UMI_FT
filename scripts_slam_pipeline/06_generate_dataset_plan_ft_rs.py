...
@click.option('-to', '--tcp_offset', type=float, default=0.14165, help="Distance from gripper tip to mounting screw") # 0.127 + 0.01465
...
def main(input, output, tcp_offset, tx_slam_tag,
    ...
    fps = None
    rows = list() # each row represents video meta data
    with ExifToolHelper() as et:
        for video_dir in video_dirs:  # each video directory represents each video folder
            mp4_path = video_dir.joinpath('raw_video.mp4')
            meta = list(et.get_metadata(str(mp4_path)))[0]
            cam_serial = meta['QuickTime:CameraSerialNumber']
            start_date = mp4_get_start_datetime(str(mp4_path)) 
            start_timestamp = start_date.timestamp() # Unix timestamp of the video
            ...
            # get realsense csv info
            rs_dir = video_dir / 'realsense_data'
            rs_csv_path = list(rs_dir.glob('*.csv'))
            if not rs_csv_path:
                continue
            
            rs_df = pd.read_csv(rs_csv_path[0])
            rs_n_frames = rs_df['frame_idx']
            secs = rs_df['secs'].to_numpy()
            nsecs = rs_df['nsecs'].to_numpy()
            rs_timestamps = secs + nsecs*1e-9
            rs_dt = np.median(np.diff(rs_timestamps))
            rs_start_timestamp = rs_timestamps[0]
            rs_end_timestamp = rs_timestamps[-1]
            ...
            rows.append({
                ...
                'rs_n_frmaes': rs_n_frames,
                'rs_dt': rs_dt,
                'rs_start_timestamp': rs_start_timestamp,
                'rs_end_timestamp': rs_end_timestamp
            })
    if len(rows) == 0:
        print("No valid videos found!")
        exit(1)
            
    video_meta_df = pd.DataFrame(data=rows) # make dataframe from rows
    ...
    for vid_idx, row in video_meta_df.iterrows():
        ...
        events.append({
            ...
            'camera_serial': row['camera_serial'],
            't': row['end_timestamp'],
            'rs_t': row['rs_end_timestamp'],
            'rs_dt': row['rs_dt'],
            'is_start': False
        })
    events = sorted(events, key=lambda x: x['t'])
    
    demo_data_list = list() # each element of demo_data_list is start and end timestamp of each video
    on_videos = set()
    on_cameras = set()
    used_videos = set()
    t_demo_start = None
    rs_t_demo_start = None
    for i, event in enumerate(events):
        ...
        if len(on_cameras) == n_cameras:
            ...
            demo_data_list.append({
                ...
                "end_timestamp": t_end,
                'rs_dt': row['rs_dt'],
                "rs_start_timestamp": rs_t_start,
                "rs_end_timestamp": rs_t_end
            })
            t_demo_start = None
    ...
    for demo_idx, demo_data in enumerate(demo_data_list):
        ...
        start_timestamp = demo_data['start_timestamp']
        end_timestamp = demo_data['end_timestamp']
        rs_dt = demo_data['rs_dt']
        rs_start_timestamp = demo_data['rs_start_timestamp']
        rs_end_timestamp = demo_data['rs_end_timestamp']

        # modify timestamp considering realsense camera
        start_timestamp = max(start_timestamp, rs_start_timestamp)
        end_timestamp = min(end_timestamp, rs_end_timestamp)
        ...
        dt = rs_dt 
        # set with realsense camera because realsense camera has lowest fps
        ...
        for cam_idx, row in demo_video_meta_df.iterrows():
            ...
            stiffness = gripper_df['stiffness'].to_numpy()
            ...
            stiffness_interp = get_interp1d(adjusted_gripper_timestamps, stiffness)
            interpolated_stiffness = stiffness_interp(video_timestamps)
            ...
            assert len(interpolated_stiffness) == n_frames
            ...
            all_stiffness.append(interpolated_stiffness)
            ...
            for cam_idx, row in demo_video_meta_df.iterrows():
                ...
                # if cam_idx < 1: # n_gripper_cams = 1
                    ...
                    grippers.append({
                        ...
                        "Stiffness": all_stiffness[cam_idx][start:end],
                ...
                rs_path = video_dir / "realsense_data"
                bag_files= list(rs_path.glob('*.rosbag'))
                if not bag_files:
                    raise FileNotFoundError(f"No rosbag in {rs_dir}")
                ...
                cameras.append({
                    ...
                    "rosbag_path": str(bag_path.relative_to(demos_dir))
                })
