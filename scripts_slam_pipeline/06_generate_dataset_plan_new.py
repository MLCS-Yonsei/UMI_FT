...
@click.option('-to', '--tcp_offset', type=float, default=0.124, help="Distance from gripper tip to mounting screw")
...
def main(input, output, tcp_offset, tx_slam_tag,
    ...
    cam_to_center_height = 0.087 # constant for UMI
    # optical center to mounting screw, positive is when optical center is in front of the mount
    cam_to_mount_offset = 0.030 # constant for GoPro Hero 9,10,11
    # cam_to_tip_offset = cam_to_mount_offset + tcp_offset
    cam_to_tip_offset = cam_to_mount_offset - tcp_offset

    # +x axis camera tilted degree : 24
    cam_to_tip_rot = 24 * (np.pi/180) 

    pose_cam_tcp = np.array([0, cam_to_center_height, cam_to_tip_offset, cam_to_tip_rot,0,0])
