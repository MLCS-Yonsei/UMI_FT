...
from umi.real_world.rtde_interpolation_compliance_controller import RTDEInterpolationComplianceController
...
class DiTEnv:
    def __init__(self, 
            ...
            sensors_config, # list of dict[{sensor_ip: XXX, sensor_port: 8890, sesnor_obs_latency: 0.001}]
            # env params
            frequency=20,
            # obs
        ...
        print("Setting robot, gripper and sensor...")

        robots: List[RTDEInterpolationComplianceController] = list()
        ...
        for rc in robots_config:
            if rc['robot_type'].startswith('ur3'):
                ...
                this_robot = RTDEInterpolationComplianceController(
                    shm_manager=shm_manager,
                    robot_ip=rc['robot_ip'],
                    frequency=500 if rc['robot_type'] == 'ur3e' else 125,
                    lookahead_time=0.1,
                    gain=300,
    ...
    def exec_compliant_actions(self,
            obs_dict: dict,
            actions: np.ndarray, 
            timestamps: np.ndarray,
            compensate_latency=False):
        ...
        # wrench and current pose
        pos = obs_dict['eef_pos']
        rot = obs_dict['eef_rot']
        current_pose = np.concatenate([pos, rot])
        force = obs_dict['force']
        torque = obs_dict['torque']
        ...
        for i in range(len(new_actions)):
            for robot_idx, (robot, gripper, rc, gc) in enumerate(zip(self.robots, self.grippers, self.robots_config, self.grippers_config)):
                ...
                r_actions = new_actions[i, 7 * robot_idx + 0: 7 * robot_idx + 6]
                g_actions = new_actions[i, 7 * robot_idx + 6]
                stiffness = new_actions[i, -1] # we should change the action dim
                robot.schedule_waypoint_compliance(
