...
class RTDEInterpolationComplianceController(mp.Process):
    """
    ...
    def __init__(self,
        ...
        self.M = np.eye(6) * 0.05

        self.Kp = dict()
        self.Kr = dict()
        
        self.Kp['high'] = np.eye(3) * 500
        self.Kp['mid'] = np.eye(3) * 300
        self.Kp['low'] = np.eye(3) * 100

        self.Kr['high'] = np.eye(3) * 500
        self.Kr['mid'] = np.eye(3) * 300
        self.Kr['low'] = np.eye(3) * 100
    ...
    def hat_map(self, x):
        # 6-vector twist [v; w] -> 4x4 se(3) matrix
        v = x[:3]
        w = x[3:]
        w_hat = self.skew_map(w)
        X = np.zeros((4, 4))
        X[:3, :3] = w_hat
        X[:3,  3] = v
        return X
    ...
    def vee_map(self, X):
        # 4x4 se(3) matrix -> 6-vector twist [v; w]
        w_hat = X[:3, :3]
        v = X[:3, 3]
        w = np.array([w_hat[2, 1], w_hat[0, 2], w_hat[1, 0]])
        return np.concatenate([v, w])

    def get_vel(self, k=None):
        return self.get_state(k, 'ActualTCPSpeed')

    def get_vel_body(self):
        V_s = self.get_vel()
        g = self.get_pose()
        r = g[:3, :3]
        p = g[:3,  3]
        # Ad_g^{-1} = [Rᵀ,  -Rᵀ skew(p); 0, Rᵀ]
        Ad_inv = np.zeros((6, 6))
        Ad_inv[:3, :3] = r.T
        Ad_inv[:3, 3:] = -r.T @ self.skew_map(p)
        Ad_inv[3:,  3:] = r.T
        return Ad_inv @ V_s

    def get_pose(self, k=None):
        pose = self.get_state(k, 'ActualTCPPose')
        g = np.eye(4)
        g[:3,:3] = rot.from_rotvec(pose[3:]).as_matrix()
        g[:3,3] = pose[:3]
        return g
    ...
    def stiffness_map(predicted_stiffness):
        if predicted_stiffness >= 0.4:
            return 'high'
        elif predicted_stiffness >= -0.4:
            return 'mid'
        else:
            return 'low'
    

    def calculate_control(self, current_pose, target_pose, wrench, stiffness):
        g = current_pose
        p, r = g[:3, 3], g[:3, :3]

        p_d, r_d = target_pose[:3], rot.from_rotvec(target_pose[3:]).as_matrix()

        v_b = self.get_vel_body()

        f_e = self.get_wrench(wrench)

        stiffness = self.stiffness_map(stiffness)

        Kp = self.Kp[stiffness]
        Kr = self.Kr[stiffness]
        K = np.block([
            [Kp, np.zeros((3,3))],
            [np.zeros((3,3)), Kr]
            ])
        ...
        f_p = r.T @ r_d @ Kp @ r_d.T @ (p - p_d)
        f_r = Kr @ (r_d.T @ r) - (r.T @ r_d) @ Kr
        f_r = np.array([f_r[2, 1], f_r[0, 2], f_r[1,0]])
        f_g = np.concatenate([f_p, f_r])

        v_b_d = v_b + self.dt * np.linalg.inv(self.M) @ (-Kd @ v_b - f_g + f_e)

        g_next = g @ expm(self.hat_map(v_b_d) * self.dt)

        R_cmd = g_next[:3, :3]
        p_cmd = g_next[:3,  3]
        r_cmd_vec = rot.from_matrix(R_cmd).as_rotvec()
        pose_cmd = np.concatenate([p_cmd, r_cmd_vec])
        return pose_cmd
    ...
    def schedule_waypoint_compliance(self, target_pose, current_pose, wrench, stiffness, target_time):
        target_pose = np.array(target_pose)
        assert target_pose.shape == (6,)
        target_compliant_pose = self.calculate_control(current_pose=current_pose, target_pose=target_pose, wrench=wrench, stiffness=stiffness)
        message = {
                'cmd': Command.SCHEDULE_WAYPOINT.value,
                'target_pose': target_compliant_pose,
                'target_time': target_time
            }
        self.input_queue.put(message)
