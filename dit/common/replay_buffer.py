...
class RobobufReplayBuffer(Dataset):
    def __init__(
        ...
        self.normalizer = pkl.load(open(normalizer_path, "rb"))
    ...
    def __getitem__(self, idx):
        ...
        imgs_obs_dict = {
            ...
            "width": o_t[..., 15:16],
            "images": i_t["cam0"],
            "depth_images": i_t["cam1"],
        }
        norm_imgs_obs_dict = self.normalizer.normalize(imgs_obs_dict)

        i_t["cam0"] = norm_imgs_obs_dict["images"].detach()
        i_t["cam1"] = norm_imgs_obs_dict["depth_images"].detach()

        o_t = np.concatenate(
            (
                norm_imgs_obs_dict["pos"].detach().numpy(),
                norm_imgs_obs_dict["angle"].detach().numpy(),
                norm_imgs_obs_dict["start"].detach().numpy(),
                norm_imgs_obs_dict["width"].detach().numpy(),
            ),
            axis=0,
        )

        a_t = self.normalizer["action"].normalize(a_t)
