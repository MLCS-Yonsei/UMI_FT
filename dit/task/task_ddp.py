...
from dit.common.replay_buffer import IterableWrapper

_TEST_WORKERS = 4

def _worker_init_fn(worker_id):
    """Worker initialization function that can be pickled"""
    np.random.seed()


def _build_data_loader(buffer, batch_size, num_workers, is_train=False,
                       rank=0, world_size=1, use_distributed_sampler=False):
    ...
    if use_distributed_sampler and world_size > 1 and not isinstance(buffer, IterableDataset):
...
class DefaultTaskDDP:
    def __init__(
        ...
        if test_buffer is not None:
            ...
            self.test_loader = _build_data_loader(
                test_buffer, 
                batch_size, 
                test_workers,
                rank=rank,
...
class BCTaskDDP(DefaultTaskDDP):
    def eval(self, trainer, global_step):
        losses = []
