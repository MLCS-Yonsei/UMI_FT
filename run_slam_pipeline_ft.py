...
def main(session_dir, calibration_dir):
    ...
    for session in session_dir: # each session represent each video
        session = pathlib.Path(os.path.expanduser(session)).absolute()

        print("############## 00_process_videos #############")
        # script_path = script_dir.joinpath("00_process_videos.py")
        script_path = script_dir.joinpath("00_process_videos_ft.py")
        assert script_path.is_file()
        cmd = [
            'python', str(script_path),
            str(session)
        ]
        result = subprocess.run(cmd) # python 00_process_videos.py <session_dir>
        ...
        result = subprocess.run(cmd) # python 01_extract_gopro_imu.py <session_dir> # this script require docker image
        assert result.returncode == 0
        ...
        script_path = script_dir.joinpath("06_generate_dataset_plan_ft.py")
        assert script_path.is_file()
        cmd = [
            'python', str(script_path),
            '--input', str(session)
